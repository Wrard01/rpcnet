...
def s64():
    lbl2.config(text='Archivo no encontrado en System32', fg='black')
    basepath = 'C://Windows//System32//'
    with os.scandir(basepath) as entries:
        for entry in entries:
            if entry.name == "rpcnet.exe":
                lbl2.config(text=entry.name + ' se encontro en System32', fg='red')
...
def s32():
    lbl2.config(text='Archivo no encontrado en SysWow64', fg="black")
    basepath1 = 'C://Windows//SysWow64//'
    with os.scandir(basepath1) as entries:
        for entry in entries:
            if entry.name == "rpcnet.exe":
                lbl2.config(text=entry.name + ' se encontro en SysWow64', fg='red')
...
def cls():
    lbl2.config(text='Preciona un boton para buscar', fg='black')
...
lbl2 = tkinter.Label(top, text="Preciona un boton para buscar")
